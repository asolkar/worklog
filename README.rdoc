...
* To work with Heroku you will need to create an account on http://heroku.com (Heroku).
  You will also need the Heroku command line client which can be obtained from
  https://toolbelt.heroku.com (Heroku Toolbelt) site. You can also get the
  client via the +heroku+ gem (although this is not recommended):
...
* Upon logging in you can create an app. It is best to first navigate into
  ...
  _http://agile-ravine-7806.herokuapp.com_
  in this case
...
* _Worklog_ supports both native accounts and Google+ Sign-in to authenticate
  ...
  https://code.google.com/apis/console (API Console).
...
* At this point, you are ready to deploy your app
  ...
  Note that some errors at +rake assets:precompile+ stage are expected
